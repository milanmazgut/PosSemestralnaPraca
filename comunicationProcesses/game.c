...
void player_roll_dice(game *gamePtr, player* currentPlayer, char* output) {
    ...
    if ( dice_1 == dice_2) {
        change_animal_ownership(&gamePtr->shop, currentPlayer, dice_1, 1);
        ...
        if(dice_1 == FOX || dice_2 == FOX) {
            if (currentPlayer->playerAnimals[SMALL_DOG] == 1) {
                ...
                change_animal_ownership(&gamePtr->shop, currentPlayer, SMALL_DOG, -1);
            } else {
                snprintf(output, BUFFER_SIZE, "You dropped %s and %s \n> Fox has eaten all your rabbits\n", animalNames[dice_1], animalNames[dice_2]);
                change_animal_ownership(&gamePtr->shop, currentPlayer, RABBIT, -currentPlayer->playerAnimals[RABBIT]);
            }
        }
        if(dice_1 == WOLF || dice_2 == WOLF) {
            if (currentPlayer->playerAnimals[BIG_DOG] == 1) {
                ...
                change_animal_ownership(&gamePtr->shop, currentPlayer, BIG_DOG, -1);
            } else {
                snprintf(output, BUFFER_SIZE, "You dropped %s and %s \n> Wolf has eaten all your sheeps, pigs and cows\n", animalNames[dice_1], animalNames[dice_2]);
                change_animal_ownership(&gamePtr->shop, currentPlayer, SHEEP, -currentPlayer->playerAnimals[SHEEP]);
                change_animal_ownership(&gamePtr->shop, currentPlayer, PIG, -currentPlayer->playerAnimals[PIG]);
                change_animal_ownership(&gamePtr->shop, currentPlayer, COW, -currentPlayer->playerAnimals[COW]);
            }
        }
    }
}
